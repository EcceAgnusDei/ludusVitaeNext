...
export type { GridCoord } from "@/features/game/lib/grid-types";
...
function setCellAliveVisual(
  ...
  root
    ...
    ?.classList.toggle("lv-cell--alive", isAlive);
}
...
export const Grid = forwardRef<GridHandle, GridProps>(function LifeGrid(
  ...
  const [alive, setAlive] = useState<Set<string>>(() => {
    ...
    return new Set(
      initialAliveCells.map((c) => cellKey(Math.trunc(c.x), Math.trunc(c.y))),
    );
  });
  ...
  }, [clearTimer, advanceOneGeneration]);
  ...
  }, [gridSize, playable, toggleCell, giveBirth]);
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      resize: (value) => {
        if (typeof value === "string") {
          ...
          const next = { x: value.x, y: value.y };
    ...
    [advanceOneGeneration, clearTimer, giveBirth],
  );
